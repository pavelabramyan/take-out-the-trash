...
const LEVELS := [
	{
		...
		"hint_ru": "Жёлтая метка → вниз по лестнице → открытая дверь во двор → помойка (E).",
		"hint_en": "Yellow mark → stairs down → open door to yard → dumpster (E).",
		"note_ru": "Записка: дверь подъезда всегда открыта наружу.",
		"note_en": "Note: the entrance door stays open to the yard.",
